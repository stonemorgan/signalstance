...
- **Test count** — full suite is now 122 tests (was 97), still ~3-4 seconds, no external dependencies. New tests cover multi-insight history grouping, empty history, batch carousel lookup (and empty short-circuit), and feeds-with-counts (including zero-article feeds and the enabled filter).
