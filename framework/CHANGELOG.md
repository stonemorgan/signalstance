...
- **Test count** — full suite is now 136 tests (was 97), still ~3-4 seconds, no external dependencies. New tests cover multi-insight history grouping, empty history, batch carousel lookup (and empty short-circuit), feeds-with-counts (including zero-article feeds and the enabled filter), `_xml_wrap` defang behavior, sanitized 500 responses (synthetic `AttributeError` injection check), preservation of ValueError messages, `_save_drafts` persistence + missing-`angle` fallback, and `require_api_key` short-circuit + pass-through.
