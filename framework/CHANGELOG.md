...
### Added
- **`tenants/taylor-morgan/PROMPT_TUNING_GUIDE.md`** — self-contained working doc originally created for hand-tuning the 10 prompt files in `tenants/taylor-morgan/prompts/` that started byte-identical to `_template/` (4 category, 3 carousel, autopilot, url_react, feed_react). The intake script only synthesizes `base_system.md`; the others started by copying the template's `<!-- AUTHORED SECTION -->` placeholder scaffolding verbatim, which reached Claude on every generation. The guide inlines Signal Stance voice DNA, four calibrated voice samples, an engine-routing map, a Dana before/after pattern reference, 10 ready-to-paste Claude briefs (one per file), a workflow, and a validation checklist. Highlights the critical Signal Stance divergence from Dana: posts end on insight, never on engagement-bait CTAs — the guide warns against the easy-to-copy pattern in Dana's own prompts. Preserved post-completion as reference for future re-tuning and tenant onboarding.
- **`tenants/taylor-morgan/prompts/` hand-tuned (all 10 prompts shipped)** — replaces the template's `<!-- AUTHORED SECTION -->` placeholder scaffolding with Signal Stance-calibrated content. Commits in tuning order: `category_pattern.md` `ca2a3d6`, `category_faq.md` `178e66d`, `category_noticed.md` `eed9227`, `category_hottake.md` `930edab`, `autopilot.md` `169b85d`, `url_react.md` `70016fa`, `feed_react.md` `3ab3de4`, `carousel_tips.md` `cd0b8c0`, `carousel_beforeafter.md` `81df79f`, `carousel_mythreality.md` `d1ce325`. Three house rules enforced consistently across all 10: (1) posts end on insight or implication, never engagement-bait questions ("Agree?", "What do you think?", "Change my mind"); (2) AI behavior framed as patterns observable in others' work / market output / corpus, never as personal practice ("I use AI..." forbidden, even where the topic is AI usage); (3) carousel CTAs point to signalstance.com per `business_config.default_ctas`, not "follow for more." Worked examples in carousel prompts use senior-pro / fractional / founder personas (LinkedIn headlines, executive summaries, fractional CFO bios) — not entry-level material.
- **Tenant intake schema and CLI extraction script** — structured onboarding flow for new tenants. Authors fill in four markdown files describing identity, voice samples, content rhythm, and brand/feeds; `scripts/intake_tenant.py` reads them, calls Claude five times (four targeted JSON extractions + one voice-profile synthesis), assembles `business_config.json`, validates it via `framework/validate.py`, and writes a ready-to-run `tenants/<name>/`.
  - **Schema:** `tenants/_intake_template/` ships five files: `01-identity.md` (name, niche, audience, credentials), `02-voice-samples.md` (sample posts, voice notes, anti-patterns), `03-content-rhythm.md` (categories, posting cadence, CTAs), `04-brand-and-feeds.md` (colors, fonts, RSS feeds or topic hints), and `README.md` (workflow, required vs. optional, the `default`/`auto` reserved values).
  - **Script:** `scripts/intake_tenant.py` — supports `--dry-run` (preview without writing), `--force` (overwrite existing tenant), and prints a readiness report flagging empty feeds, voice-sample count, and which prompt files were copied unchanged from `_template/`.
  - **Voice synthesis honors tenant-specific anti-patterns** — e.g. forbids engagement-bait CTAs when the intake declares them off-limits. Distills Voice notes / Anti-patterns sections directly into the Hard Rules and CTA sections of the synthesized `prompts/base_system.md`.
  - **Reserved values:** `default` keeps a `_template/` value, `auto` lets the LLM propose one. `<!-- review note -->`-style blockquotes in the intake markdown are ignored as source data so authors can leave themselves notes inline.
- **`framework/validate.py`** — pure validator for the `business_config.json` schema, extracted from `business_config.py` so tools that need to validate a freshly built config (the intake script) can do so without triggering business_config's import-time tenant load. `ConfigError`, `REQUIRED_KEYS`, `VALID_DAYS`, and `validate_business_config()` (renamed from private `_validate`) are now publicly exported. No behavior change — `business_config.py` imports them and calls `validate_business_config()` at module load time. 166 framework tests pass unchanged.
- **`tenants/taylor-morgan/`** — new tenant generated end-to-end via the intake flow. Sourced from the Signal Stance Personal LinkedIn profile, company page content, strategic plan, and content playbook (external Playbook directory). Voice samples are the four "good example" posts from the playbook, used as calibrated voice target until real published posts accumulate. Schedule is Mon–Thu (Friday left as flex per the playbook's "skip if not ready" rule). Default CTAs use "More writing at signalstance.com" framings instead of follow-for-more language, per playbook anti-patterns. `feeds.json` is empty — only topic hints supplied at intake time.
- **`intake/taylor-morgan/`** — the four filled-in intake markdown files for taylor-morgan, kept under version control as the reproducible source-of-truth that produced the tenant. Re-running `python scripts/intake_tenant.py taylor-morgan --from intake/taylor-morgan --force` regenerates the tenant from these files.
